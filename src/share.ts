...
export default class Share {
  public static async getShare(
    ...
    const share: Share = new Share(client, shareType, permissions, id, user ?? null);
    await share.initialize();
    return share;
  }

  public static createShareRequestBody(createShare: ICreateShare): any {
    ...
    if (shareType === ShareType.user || shareType === ShareType.group) {
      if (createShare.shareWith == null) {
        throw new ClientError(
          "Error, when using ShareType user or usergroup, you need to specify shareWith",
          "ERR_SHARETYPE_USER_GROUP_WITHOUT_SHAREWITH",
        );
      }
    }
    ...
    const shareRequest: {
      ...
      //  @todo    permissions: 1,
      shareType,
      permissions: createShare.permissions,
    };
    ...
    for (const key of Object.keys(shareRequest)) {
      theFormData.append(key, shareRequest[key as keyof typeof shareRequest] as string);
    }
