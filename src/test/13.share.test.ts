...
import "mocha";
import { Client, File, Folder, ICreateShare } from "../client";
import FakeServer from "../fakeServer";
import RequestResponseLogEntry from "../requestResponseLogEntry";
import Share, { SharePermission, ShareType } from "../share";
...
describe("13-NEXCLOUD-NODE-CLIENT-SHARE", function () {
  ...
  beforeEach(async function () {
    if (this.currentTest && this.currentTest.parent) {
      client = await getNextcloudClient(
        this.currentTest.parent.title + "/" + this.currentTest.title
      );
    }
  });
  ...
  it("01 create public share", async () => {
    ...
    try {
      ...
      try {
        ...
        expect(e.message, "expect no exception setting password").to.be.equal(
          null
        );
      }
      ...
      try {
        ...
        expect(e.message, "expect no exception setting expiration").to.be.equal(
          null
        );
      }
      ...
      try {
        ...
        expect(e.message, "expect no exception deleting share").to.be.equal(
          null
        );
      }
    } catch (e) {
    ...
    try {
      ...
      expect(e.message, "expect no exception 'publicUpload:true'").to.be.equal(
        null
      );
    }
  ...
  it("02 invalid share responses", async () => {
    ...
    expect(errorCode, "expect an exception").to.be.equal(
  ...
  it("03 create public share for folder", async () => {
    ...
    const createShare: ICreateShare = {
